...
- **Harnesses**: which CLIs (and models) are in the maker/checker rotation is config, not code — `LOOP_KIT_HARNESSES` in `loop.config.sh` (space-separated *members*, default `codex claude cursor`). A member is either a bare harness name (`cursor`, using that harness's own configured default model) or `harness:model` (`cursor:grok-4.5-high`), pinning a specific model — so a multi-model-family harness like `cursor` or `opencode` can fill more than one seat by itself, e.g. `LOOP_KIT_HARNESSES="cursor:grok-4.5-high cursor:claude-4.5-sonnet"` is a valid 2-member rotation using only one CLI, since a diff made under one model family is still meaningfully checked by a different one. Every member is used as both a maker (for tasks whose `maker:` frontmatter names it) and a reviewer. Each *harness* is defined by a small adapter script, `loop/harnesses/<name>.sh` — this kit ships built-in adapters for `codex`, `claude`, and `cursor`; drop any subset (need ≥2 members total, not necessarily ≥2 distinct harnesses, and no two members may be identical), reorder them, repeat one with different pinned models, or add your own (aider, gemini-cli, opencode, a custom script) by writing an adapter against the fixed interface documented in `loop/harnesses/TEMPLATE.sh.example` (or scaffold one with `loop/new_harness.sh <name>`). Gets one atomic task per run, fresh context every time. State never lives in its head — it lives in this directory and in git.
...
- **opencode** ships an adapter, but only its `harness_council_run` function is implemented — `loop/harnesses/opencode.sh`'s `harness_maker_run`/`harness_reviewer_run` are left as TODO stubs on purpose. Fanning a question out to several models with no cross-visibility (`loop/council.sh`, see "Council" below) is a different job from a pipeline where each step is supposed to see the prior step's work, and only the council invocation has actually been exercised by this kit. Nothing stops you from filling in the maker/reviewer TODOs and adding opencode to `LOOP_KIT_HARNESSES` too, if you want that instead — just smoke-test it first.
- **Usage-limit backoff**: none of Codex/Claude/cursor-agent expose a "remaining quota" API — the only signal any of them gives is the error text printed once a daily/weekly limit is already hit. `run.sh` reacts to that: any maker or reviewer call whose output matches that class of message gets a cooldown recorded in `loop/state/backoff.txt` (`<harness> <until-epoch>`, best-effort timestamp parsed from the message, falling back to a flat 1-hour cooldown if the wording doesn't match). This is keyed by *harness*, not by member/model — a rate limit on the `cursor-agent` CLI/account blocks every member that routes through it (`cursor:grok-4.5-high`, `cursor:claude-4.5-sonnet`, ...) at once, since they share the same underlying account. Before starting a task, the loop checks whether that task's maker's harness is currently in a recorded cooldown and, if so, blocks it immediately without spending a worktree or an attempt. Delete `loop/state/backoff.txt` (or an individual harness's line) to manually clear a cooldown early.
...
loop/council.sh          fans a question out to LOOP_KIT_COUNCIL_HARNESSES in parallel, read-only — see "Council" below
loop/council_prompt.tpl.md   one template, rendered per council question and handed to whichever member is answering
```
...
`loop/review_pr.sh <pr-number> [--reviewer <member>] [--post-comment]` is another read-only counterpart to the checker step above — for a PR that never came out of `queue/` at all. It's the same job in the same shape: check out the PR's branch into its own worktree, render `pr_review_prompt.tpl.md` (a PR-flavored sibling of `review_prompt.tpl.md` — they share the exact same red-team mandate via `review_mandate.partial.md`, only the framing/subject section differs), and run it through whichever harness/member you name via `harness_reviewer_run` — the identical adapter function the queue-driven loop uses, so no new adapter code was needed to support this. `--reviewer` defaults to the first entry in `LOOP_KIT_HARNESSES`; run it again with a different one for a second independent opinion. Prints the review locally by default; `--post-comment` posts it to the PR via `gh pr comment` (requires the `gh` CLI, authenticated). See the script's own header comment for the full usage and a known limitation around branches already checked out elsewhere.
