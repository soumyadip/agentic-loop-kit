...
- Do not modify files under `loop/`, or any doc this project treats as governance-sensitive (see `{{PRINCIPLES_DOC}}` / the sensitive-path pattern in `loop/run.sh`) — those are out of scope for any task that doesn't explicitly name them.
