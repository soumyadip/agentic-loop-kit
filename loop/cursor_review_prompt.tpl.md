...
You are running in `plan` mode: you can read files, search, and run read-only commands (build, lint, test, `git diff`), but you cannot edit anything. If you believe something needs fixing, describe it precisely enough that another agent could fix it from your description alone — you will not get the chance to fix it yourself in this pass.
