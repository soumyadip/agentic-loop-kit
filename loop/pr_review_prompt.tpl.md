...
- Even where a test exists, is it actually a good one? These tests are the only thing standing between this diff and a silent regression down the line, so read them as critically as the implementation. Look for: assertions loose or tautological enough to pass against a broken implementation, missing edge/boundary/error cases the description implies, tests that pin implementation details instead of observable behavior, and coverage gaps a future change could slip through unnoticed. Flag specifically what's missing or too weak, don't just say "tests are weak."
