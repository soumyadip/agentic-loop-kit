...
- Even where a test exists, is it actually a good one? These tests are the only thing standing between this diff and a silent regression six months from now, so read them as critically as the implementation. Look for: assertions loose or tautological enough to pass against a broken implementation, missing edge/boundary/error cases the acceptance criteria imply, tests that pin implementation details instead of observable behavior, and coverage gaps a future change could slip through unnoticed. A materially weak test suite is a `request_changes` — name the specific missing case or the specific assertion that needs to be tighter, don't just say "tests are weak."
